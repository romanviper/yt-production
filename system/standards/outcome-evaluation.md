...
- **Listening experience:** without visual annotation, can the listener follow what is encountered or changing?
...
Do not require invented drama or an event chain as the fix. Any evidenced carrier can create movement if the audience can follow what changes.

## Diagnose the layer

- `prose_execution`: intent/evidence are sound; wording, pacing, redundancy or narrator stance failed.
- `local_design`: audience shift, carrier or evidence selection cannot produce the experience.
- `product_architecture`: section boundary or whole-script movement design is wrong.
- `evidence`: needed claim/material is missing, contradicted or insufficient.

Route to the smallest responsible layer with an observable acceptance test. Do not create a global writer rule for a one-off failure.
